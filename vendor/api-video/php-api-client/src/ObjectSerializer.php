...
class ObjectSerializer
{
    ...
    public static function deserialize($data, $class, $httpHeaders = null)
    {
        ...
        if ($class === '\SplFileObject') {
            ...
            $discriminator = $class::getDefinition()->discriminator;
            if (!empty($discriminator) && isset($data->{$discriminator}) && is_string($data->{$discriminator})) {
                $subclass = '\ApiVideo\Client\Model\\' . $data->{$discriminator};
            ...
            $instance = new $class();
            foreach ($instance::getDefinition()->openAPITypes as $property => $type) {
                $propertySetter = $instance::getDefinition()->setters[$property];

                if (!isset($propertySetter) || !isset($data->{$instance::getDefinition()->attributeMap[$property]})) {
                    continue;
                }

                if (isset($data->{$instance::getDefinition()->attributeMap[$property]})) {
                    $propertyValue = $data->{$instance::getDefinition()->attributeMap[$property]};
